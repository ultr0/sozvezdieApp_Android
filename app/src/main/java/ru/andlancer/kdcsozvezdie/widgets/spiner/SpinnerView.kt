package ru.andlancer.kdcsozvezdie.widgets.spiner
...
class SpinnerView<DataType> : AppCompatTextView {
    ...
    private var mAdapter: SpinnerViewBaseAdapter<DataType>? = null
    ...
    fun setAdapter(adapter: SpinnerViewBaseAdapter<DataType>) {
        mAdapter = adapter
    ...
    private fun init(context: Context, attrRes: AttributeSet?) {
        ...
        context.obtainStyledAttributes(attrRes, R.styleable.SpinnerView)?.run {
            mDropDownItemHeight = getLayoutDimension(R.styleable.SpinnerView_dropdown_item_height, WindowManager.LayoutParams.WRAP_CONTENT)
            mDropDownListMaxHeight = getDimensionPixelSize(R.styleable.SpinnerView_dropdown_max_height, WindowManager.LayoutParams.WRAP_CONTENT)
            mCollapseDrawable = getResourceId(R.styleable.SpinnerView_collapse_item_display_background, -1)
            mExpandDrawable = getResourceId(R.styleable.SpinnerView_expand_item_display_background, -1)
            if (hasValue(R.styleable.SpinnerView_drop_down_list_background)) {
                mDropDownListBackgroundDrawable = getDrawable(R.styleable.SpinnerView_drop_down_list_background)
            }

            if (hasValue(R.styleable.SpinnerView_arrow_icon)) {
                mArrowDrawable = getDrawable(R.styleable.SpinnerView_arrow_icon)
                setCompoundDrawablesWithIntrinsicBounds(null, null, mArrowDrawable, null)
            }

            mHintTextEnable = getBoolean(R.styleable.SpinnerView_hint_text_enable, false)
            this.recycle()
        }
    ...
    private fun calculatePopupWindowHeight(): Int {
        ...
        var listViewHeight = mAdapter?.itemCount ?: 1 * itemHeight
